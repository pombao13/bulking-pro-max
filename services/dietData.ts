...
// (migrated from diet-data.js)
// ══════════════════════════════════════════════
import type { NutritionalData, IngredientGroup, Ingredient, Meal, Refeicoes } from '@/types';

export const DB: Record<string, NutritionalData> = {
...
export function calcI(id: string, amount: number): Ingredient | null {
  const d = DB[id];
  if (!d) return null;
...
export function mkMeal(nome: string, hora: string, icon: string, pairs: [string, number][]): Meal {
  const ingrs = pairs.map(([id, amt]) => calcI(id, amt)).filter(Boolean) as Ingredient[];
  const mac = ingrs.reduce((a, b) => ({
...
const SB: [string, number][] = [['farinha_arroz', 20], ['farinha_amend', 20], ['polvilho', 20], ['beterraba', 15], ['levedo', 15], ['whey', 30]];
const ST: [string, number][] = [...SB, ['banana', 200], ['mel', 20]];

function buildFase(ar: number, fr: number, ma1: number, ma2: number, pu: number, ov: number, az: number, shTrab = 'SB', shFolga = 'SB') {
  ...
  return {
    ...
    folga: [
      ...
      mkMeal('Arroz + Banana',    '22:30', '🍚', [['arroz', 100], ['banana', 100], ...(shTrab === 'ST' ? [['mel', 15] as [string, number]] : [])]),
    ],
  };
}

export function getDefaultRefeicoes(): Refeicoes {
  return {
...
export function calcSuplDiario(s: { unidade: string; preco_total: number; qtd_total: number; qtd_diaria: string | number }): number {
  const preco_total = parseFloat(String(s.preco_total)) || 0;
  const qtd_total = parseFloat(String(s.qtd_total)) || 0;
  const qtd_diaria = parseFloat(String(s.qtd_diaria)) || 0;
